...
| **样式** | CSS (CSS 变量 + BEM 命名) | 暗色为主，像素风时钟，青色 accent |
...
Dark Mode:
  ...
  --text:      #e0e0e0        主文本
  --text-dim:  #4a4a4a        次要文本
  --accent:    #00d4aa        青色高亮（冷调电台感）
  --green:     #00d4aa        在线/活跃指示
  --red:       #ff4757        收藏红
  --border:    #1a1a1a        分隔线
  --grid-color: rgba(0,212,170,0.015)  点阵网格
...
Light Mode:
  ...
  --green:     #00b894
  --border:    #e2ddd0
  --grid-color: rgba(0,184,148,0.015)  点阵网格
```

核心调性：**深黑 + 青色点缀、极简冷调、点阵网格背景**，不是温暖感，是克制感。

### 布局

从上到下纵向排列，单栏居中，app 容器 `height: 100vh` + `overflow: hidden`，主要内容包裹在 `.app__card` 卡片容器内（1px 边框，极简冷调），聊天区内部滚动：

```
┌──────────────────────────────────────┐
│  🎙 TREELIO              [LIGHT]     │  ← 顶栏（点击头像 → 打开 Profile 抽屉）
├──────────────────────────────────────┤
│  ┌─ .app__card ──────────────────┐ │
│  │       21:11                       │ │  ← 像素时钟 (VT323)，带点阵背景
│  │      Monday                       │ │
│  │     30 APR 2026                  │ │
│  │        ● ON AIR                  │ │
│  ├──────────────────────────────────┤ │
...
- [x] Dark/Light 主题（深黑 + 青色 accent，点阵网格背景）
- [x] 播放历史记录
- [x] 收藏状态持久化
- [x] 天气 API 路由
- [x] Profile 页（右侧抽屉弹出，展示品味标签 + 播放统计）
- [x] 无版权歌曲自动过滤（后端过滤，不展示不可播歌曲）
