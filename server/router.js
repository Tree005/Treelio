...
// 注意：排除"来两首/来几首...放两首/放几首..."（这些是 PLAYLIST）
const playNowMatch = text.match(/^(?:再?(?:播放|播|放(?!再?[几两三四五六七八九十\d]+首)|播一下|放一下)|来(?!再?[几两三四五六七八九十\d]+首)(?:再?)?(?:首|一首)|放(?!再?[几两三四五六七八九十\d]+首)(?:再?)?(?:首|一首))\s*(.+)/i);
if (playNowMatch) {
  return {
    intent: INTENT.PLAY_NOW,
